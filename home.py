...
def trigger_state_changed(entity, new_state, old_state):
    for f in state_changed_callbacks:
        try:
            ...
            custom_logging.log(__name__, "Error while calling StateChanged callback %s: %s" % (str(f), str(ex)))
...
def trigger_event(entity, data):
    for f in event_callbacks:
        try:
            ...
            custom_logging.log(__name__, "Error while calling Event callback %s: %s" % (str(f), str(ex)))
...
def trigger_state_initialized():
    for f in state_initialized_callbacks:
        try:
            ...
            custom_logging.log(__name__, "Error while calling StateInitialized callback %s: %s" % (str(f), str(ex)))
...
async def main():
    ...
    next_id += 1
    await websocket.send(json.dumps(
        {'id': get_state_msg_id, "type": "get_states"}
    ))
    ...
    while not state_initialized:
        ...
        if (debug):
            custom_logging.log(__name__, dic)
    ...
    next_id += 1
    await websocket.send(json.dumps(
        {'id': subscribe_id, 'type': 'subscribe_events', 'event_type': 'deconz_event'}
    ))
